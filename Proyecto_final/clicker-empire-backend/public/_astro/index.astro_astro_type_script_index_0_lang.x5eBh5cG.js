...
async function l(e = 1) {
    try {
        const o = await fetch(`${config.apiUrl}/api/forum/comments?category_id=1&page=${e}`, {
            credentials: "include",
        ...
        i && (i.textContent = e.toString()), window.history.pushState({}, "", `/forum/seccion1?page=${e}`), L();
...
_ && _.addEventListener("submit", async e => {
    ...
    try {
        if (!(await fetch(`${config.apiUrl}/api/forum/comments`, {
            ...
            credentials: "include",
            body: JSON.stringify({content: i.value, category_id: 1})
